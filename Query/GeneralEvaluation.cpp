...
bool GeneralEvaluation::parseQuery(const string &_query)
{
	...
	try
	{
		this->query_parser.SPARQLParse(_query, this->query_tree,tp);
	}
	catch (const char *e)
	{
		printf("%s\n", e);
...
bool GeneralEvaluation::decompose_query(const string &_query,vector<query_block>& qb)
{
	vector<tri> tp;
	try
	{
	...
	{
		cout<<tp[i].subject<<" "<<tp[i].object<<" "<<tp[i].st<<" "<<tp[i].ed<<" ";
		for(int j=0;j<tp[i].predicate.size();j++)
			cout<<tp[i].predicate[j]<<" ";
		cout<<endl;
	}

	map<string,int> str_id;
	vector<string> id_str;
	id_str.push_back(string());
	int id_cnt=0;

	for(int i=0;i<tp.size();i++)
	{
		if(str_id.count(tp[i].subject)==0)
			str_id[tp[i].subject]=++id_cnt,id_str.push_back(tp[i].subject);
		if(str_id.count(tp[i].object)==0)
			str_id[tp[i].object]=++id_cnt,id_str.push_back(tp[i].object);
	}

	vector<int>fa(id_cnt+1);
	for(int i=1;i<=id_cnt;i++)fa[i]=i;

	for(int i=0;i<tp.size();i++)if(tp[i].st==-1&&tp[i].predicate.size()==1)
	{
		int x=str_id[tp[i].subject],y=str_id[tp[i].object];
		while(x!=fa[x])x=fa[x];
		while(y!=fa[y])y=fa[y];
		fa[y]=x;
	}
	...
	int block_cnt=0;
	map<int,int>fa_block;
	for(int i=1;i<=id_cnt;i++)
	{
		int x=i;
		while(x!=fa[x])x=fa[x];
		if(fa_block.count(x)==0)
			fa_block[x]=++block_cnt;
	}

	vector<set<int> >id_block(id_cnt+1,set<int>());
	qb.push_back(query_block());
	for(int i=0;i<tp.size();i++)if(tp[i].st==-1&&tp[i].predicate.size()==1)
	{
		int x=str_id[tp[i].subject],y=str_id[tp[i].object];
		int b=x;
		while(qb.size()<=b)qb.push_back(query_block());
		while(b!=fa[b])b=fa[b];
		b=fa_block[b];
		qb[b].edge.push_back(tp[i].subject+"\t"+tp[i].predicate[0]+"\t"+tp[i].object);
		qb[b].st=qb[b].ed=-1;
		qb[b].pro_path=0;
		id_block[x].insert(b),id_block[y].insert(b);
	}
	...
	for(int i=0;i<tp.size();i++)if(tp[i].st!=-1||tp[i].predicate.size()>1)
	{
		qb.push_back(query_block());
		for(int j=0;j<tp[i].predicate.size();j++)
			qb[qb.size()-1].edge.push_back(tp[i].subject+"\t"+tp[i].predicate[j]+"\t"+tp[i].object);
		qb[qb.size()-1].st=tp[i].st;qb[qb.size()-1].ed=tp[i].ed;
		qb[qb.size()-1].pro_path=1;
		int x=str_id[tp[i].subject],y=str_id[tp[i].object];
		id_block[x].insert(qb.size()-1),id_block[y].insert(qb.size()-1);
	}

	for(int i=1;i<=id_cnt;i++)
	{
		for(set<int>::iterator it1=id_block[i].begin();it1!=id_block[i].end();it1++)
			for(set<int>::iterator it2=id_block[i].begin();it2!=id_block[i].end();it2++)
				if(*it1!=*it2)qb[*it1].connection.push_back(make_pair(id_str[i],*it2));
	}
