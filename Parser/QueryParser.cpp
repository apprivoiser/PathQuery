...
void QueryParser::SPARQLParse(const string &query, QueryTree &querytree,std::vector<tri>& tp)
{
...
int QueryParser::printNode(pANTLR3_BASE_TREE node,std::vector<tri>& tp,int dep)
{
	...
	if(treeType==177)//subject
	{
		tp.push_back(tri());
		tp[tp.size()-1].st=tp[tp.size()-1].ed=-1;
		pANTLR3_BASE_TREE childNode = (pANTLR3_BASE_TREE) node->getChild(node, 0);
		tp[tp.size()-1].subject=(char *)childNode->getText(childNode)->chars;
		return 0;
	}
	else if(treeType==119)//object
	{
		pANTLR3_BASE_TREE childNode = (pANTLR3_BASE_TREE) node->getChild(node, 0);
		tp[tp.size()-1].object=(char *)childNode->getText(childNode)->chars;
		return 0;
	}
	// else if(treeType==200)
	// {
	// 	if(tp[tp.size()-1].subject.compare("SUBJECT")==0)
	// 		tp[tp.size()-1].subject=s;
	// 	else
	// 		tp[tp.size()-1].object=s;
	// }
	else if(treeType==131)//predicate
	{
		pANTLR3_BASE_TREE childNode = (pANTLR3_BASE_TREE) node->getChild(node, 0);
		tp[tp.size()-1].predicate.push_back((char *)childNode->getText(childNode)->chars);
		return 0;
	}
	else if(treeType==83)//var_path
	{
		if(tp[tp.size()-1].st==-1)
			tp[tp.size()-1].st=tp[tp.size()-1].ed=std::atoi(s);
		else
			tp[tp.size()-1].ed=std::atoi(s);
	}

	for (unsigned int i = 0; i < node->getChildCount(node); i++)
	{
		pANTLR3_BASE_TREE childNode = (pANTLR3_BASE_TREE) node->getChild(node, i);
		hasErrorNode += printNode(childNode,tp,dep + 1);
	}
