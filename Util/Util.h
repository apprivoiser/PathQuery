...
public:
	...
	void init()
	{
		...
		preType=entityCnt=triples=0;
	}
	void loadGraph(vector<vector<vector<string> > >&candidate)
	{
		vector<pair<int,int> >tmp;
		for(int k=0;k<candidate.size();k++)
		{
			...
			{
				for(int j=0;j<2;j++)
					if(entityToID.count(candidate[k][i][j])==0)
					{
						entityToID[candidate[k][i][j]]=++entityCnt;
						IDToEntity.push_back(candidate[k][i][j]);
						cluster.push_back(tmp);
					}
				int sub=entityToID[candidate[k][i][0]],obj=entityToID[candidate[k][i][1]];
				subject_set.insert(sub);
				object_set.insert(obj);
				edge[k].push_back(make_pair(sub,obj));
				cluster[sub].push_back(make_pair(k,edge[k].size()-1));
			}
		}
		set<int>::iterator it=subject_set.begin();
		for(;it!=subject_set.end();it++)
			if(object_set.count(*it))
				sub_obj_set.insert(*it);
		// printf("triples: %d\n", triples);
		printf("entityCnt: %d\n",entityCnt);
		// printf("predicate: %d\n", preType);
...
class tree
{
	...
	public:
		...
		void output_hash(int &st,graph& test,int dep,node* nd,std::vector<std::string>& str,std::vector<std::vector<string> >&candidate)
		{
			if(!nd->children.size())return;
			// ans[dep+1]+=nd->children.size();
			if(dep<=0)str=std::vector<std::string>();
			int sz=str.size();
			for(int i=0;i<nd->children.size();i++)
			{
				int p=nd->children[i]->val.first;
				int so=nd->children[i]->val.second;
				// std::string pre=test.IDToPredicate[p];
				std::string sub=test.IDToEntity[test.edge[p][so].first];
				std::string obj=test.IDToEntity[test.edge[p][so].second];
				str.push_back(sub);
				if(dep>=0)str.push_back(obj);
				if(dep+1>=st)candidate.push_back(str);
				output_hash(st,test,dep+2,nd->children[i],str,candidate);
				str.resize(sz);
			}
		}

		void output(int &st,graph& test,int dep,node* nd,std::vector<std::string>& str,std::vector<std::vector<string> >&candidate)
		{
			if(!nd->children.size())return;
			// ans[dep+1]+=nd->children.size();
			if(!dep)str=std::vector<std::string>();
			int sz=str.size();
			for(int i=0;i<nd->children.size();i++)
			{
				int p=nd->children[i]->val.first;
				...
				std::string sub=test.IDToEntity[test.edge[p][so].first];
				std::string obj=test.IDToEntity[test.edge[p][so].second];
				if(!dep)str.push_back(sub);
				str.push_back(obj);
				if(dep+1>=st)candidate.push_back(str);
				output(st,test,dep+1,nd->children[i],str,candidate);
				str.resize(sz);
			}
		}
